...
**Supabase 연동 코드가 추가되었지만 원격 프로젝트 연결은 아직 완료되지 않았습니다.** 설정 전에는 접수를 차단합니다. 설정 후에는 서버가 사진 업로드와 DB 저장을 확인해야 접수 완료가 표시됩니다. 현재 동의 문서는 초안이어서 실제 접수는 닫혀 있고, 명시적인 로컬 가상 데이터 테스트만 허용합니다.
...
관리자 화면: `http://localhost:4174/admin.html`. Supabase Auth 사용자 생성과 관리자 권한 등록 후 사용할 수 있습니다.
설정·DB 구조·검증 범위·남은 작업은 [Supabase 연결 안내](docs/SUPABASE_SETUP.md)를 확인하세요.
`npm run test:db`로 DB 및 API 자동 검증, `npm run db:check`로 설정 후 읽기 전용 연결 점검을 실행합니다.
...
- 신청 출생연도는 올해 기준 20~40세에 해당하는 연도로 제한합니다. 생월일을 받지 않으므로 생일 경계를 포함한 정확한 만 나이 검증은 하지 않습니다. 희망 최소·최대 나이는 만 20~40세로 제한합니다.
- 키는 145~194cm와 ‘195cm이상’으로 구성하며 마지막 선택의 데이터 값은 `195_plus`입니다. 카드에도 ‘195cm이상’으로 표시합니다.
- 직업군 ‘기타’ 선택 시 `job_other` 직접 입력을 필수로 받습니다. 다른 직업군을 선택하면 숨김·비활성화하여 제출 대상에서 제외합니다.
...
- 사진 구도 편집은 브라우저 메모리에서 처리하며, 실제 제출 시 구도를 적용한 1780×1090 JPEG를 비공개 저장소에 업로드합니다. 제출 전 새로고침하면 입력값과 사진은 사라집니다. 원본 사진은 전송하지 않습니다.
...
- DB 생성 SQL 및 운영자 조회·삭제 코드는 작성했으며, 실제 Supabase 프로젝트 적용·권한·사진 저장 검증이 남아 있습니다.
- 서버에서 필수 동의·선택정보 제외를 검증하고 동의 버전과 시각을 기록합니다. 동의 문서 확정이 필요합니다.
- 연락처는 전화번호만 입력받고 카카오톡으로 소개 제안을 전달한다는 문구를 표시합니다. Instagram·카카오톡 ID 입력과 추가 연락수단 동의는 사용하지 않습니다.
- 현재 `FormData` 생성 시 별도 선택 동의 없는 종교 정보는 제외됩니다.

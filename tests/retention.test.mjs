...
test('retention cleanup reports successful purge and storage deletion', async () => {
  const calls=[];
  const backend={
    async rpc(name, body){calls.push({name,body});if(name==='daese_prepare_retention_purge')return 1;return null;},
  ...
  assert.deepEqual(await response.json(),{purged:1,removed:1,failed:0});
...
test('retention cleanup records a failed photo deletion for the next retry', async () => {
  const calls=[];
  const backend={
    async rpc(name,body){calls.push({name,body});if(name==='daese_prepare_retention_purge')return 0;return null;},
    async call(){return [{photo_path:'fake/path.jpg'}];},
    async removePhoto(){throw new Error('storage down');},
  };
  const response=await createRetentionHandler(env,{backendFactory:()=>backend})(request('test-secret'));
  assert.equal(response.status,503);
  assert.deepEqual(await response.json(),{purged:0,removed:0,failed:1});

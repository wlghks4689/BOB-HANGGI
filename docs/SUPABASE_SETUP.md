...
- **앱 연결은 아직 미완료: 원격 SQL 적용, 실제 Auth 로그인, Storage 업로드·비공개 검증, 배포는 아직 하지 않았습니다.**
...
- 운영 주체: 사용자 본인 명의. 공개 문서에 기재할 이름과 문의 이메일은 확인 필요.
- 사업자등록: 아직 하지 않은 상태(사용자 확인). PASS 제공업체의 사업자 미등록 상태에서의 이용 가능 여부, 계약·심사·요금을 확인해야 함.
- 문의 및 삭제 요청 창구: 이메일을 기본으로 하고 별도 채널 하나 추가를 검토. 실제 주소와 채널은 미정.
- 미승인 신청서: 심사 결과 안내 시점부터 48시간 보관 후 자동 삭제.
- 승인 회원: 서비스 이용 종료 또는 삭제 요청 후 48시간 이내 처리.
...
- Turnstile: Cloudflare 로그인 완료(사용자 확인). 위 도메인에 대한 위젯 생성, 사이트 키·서버 비밀 키 발급 및 앱 연결은 아직 진행하지 않음. 예정 설정은 위젯 이름 `daese-sogating`, 허용 호스트 `daese-sogating.vercel.app`, 모드 `Managed`.

위 내용은 운영 요구사항이며, 자동 삭제·본인인증·공개 동의 문서에 구현 완료된 상태가 아닙니다.
48시간 자동 삭제를 위해 결과 안내 시각, 이용 종료 시각, 삭제 요청 시각과 삭제 기한을 기록하고,
DB 개인정보와 Storage 사진을 함께 정리하는 정기 작업 및 실패 재시도를 구현해야 합니다.
심사 결과를 아직 안내하지 않은 장기 미처리 신청서의 보관 상한은 별도 결정이 필요합니다.

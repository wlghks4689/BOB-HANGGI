...
- 원격 함수 배포 및 Vercel 페이지 연결 확인. 이번 점검에서 공개 신청 API GET 응답, 대시보드 프로젝트 상태와 마이그레이션 목록을 확인했습니다. 실제 신청 제출, 관리자 Auth 로그인, 사진 업로드·비공개 접근은 이번 점검에서 실행하지 않았습니다.
...
- 확정된 보유정책이며, 자동 파기 구현 및 검증 후 실제 운영 접수를 개방한다. 현재 수동 관리자 삭제만 있으며 보유기간 자동 파기는 없습니다.
- 새 코드에는 `RETENTION_AUTOMATION_VERIFIED=false` 릴리스 차단을 추가했습니다. 이는 환경변수가 아니라 코드 상수이며, 기존 draft 차단과 함께 유지합니다. 이번 새 코드·새 SQL·정적 페이지는 아직 배포하지 않았습니다.
...
3. `supabase/migrations/202609070003_privacy_policy_v1.sql` — 이번 추가, 원격 미적용. 새 동의 버전만 허용하며 자동 파기 구현이나 접수 개방은 하지 않습니다.
...
- 현재 `js/backend-config.js`는 배포된 Supabase API를 가리킵니다. 2026-09-07 확인 시 원격은 로컬 origin 요청에 403을 반환하여 로컬 신청 화면에는 접수 서버 연결 오류가 표시됩니다. 이번에는 허용 origin을 확장하지 않았고 신청 버튼도 비활성 상태입니다. 격리 개발 테스트 시에만 프런트가 로컬 `/api/applications`를 사용하도록 별도 설정해야 하며, 로컬 실행만으로 운영 API 검증을 대체하지 않습니다.

## 5. 클라우드 함수와 정적 사이트 연결

아래는 향후 갱신 배포 시 사용할 명령입니다. 함수 자체는 이미 배포되어 있으나 이번 수정본은 아직 배포하지 않았습니다.
...
- 개인정보 문의 및 삭제 요청 창구: [wlghks1778@gmail.com](mailto:wlghks1778@gmail.com)으로 통일.
...
공개 방침과 신청 동의는 이번에 동기화했지만 자동 삭제·본인인증은 미구현입니다.
결과 안내 시각, 이용 종료 시각, 삭제 요청 시각, 삭제 기한을 기록하고 DB 개인정보와 Storage 사진을 함께 정리하는 정기 작업·실패 재시도·기한 초과 경보를 구현해야 합니다.
접수 시각 기반 미처리 30일 상한도 이 작업에 포함해야 합니다. 현재 `updated_at`이나 검토 변경 시각은 결과 안내 시각의 대체 수단이 아닙니다.
...
원격 API의 현재 버전은 `2026-09-07`입니다. 새 SQL 적용·함수 갱신·프런트 갱신·서버 `CONSENT_VERSION` 갱신은 닫힌 상태에서 함께 진행해야 합니다. 이번에는 원격 버전 변경을 수행하지 않았습니다.
정책 문서 확정만으로 `APPLICATIONS_ENABLED`를 켜거나 draft/자동 파기 차단을 제거하지 않습니다. 운영 차단 상수는 자동 파기 구현·실제 검증 및 아래 미확인 사항 해결 후 별도 변경으로 검토합니다.
...
- [ ] 48시간/30일 기한 자동 파기, 실행 스케줄, 재시도, 실패 경보 및 기한 경계 테스트. 사진이 삭제 대기열에 남은 것을 삭제 완료로 취급하지 않음.

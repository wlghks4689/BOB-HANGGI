...
현재 실제 운영 접수는 차단 상태다. `APPLICATIONS_ENABLED=false`이며, 서버 코드의 `RETENTION_AUTOMATION_VERIFIED=false` 조건 때문에 운영 환경변수만 바꿔도 접수가 열리지 않는다. 로컬 가상 데이터 테스트 예외는 Edge 환경에서 적용되지 않는다.
...
보유기간에 따른 자동 파기는 현재 미구현이다. 결과 안내 시각, 이용 종료 시각, 삭제 요청 시각과 삭제 기한을 기록하는 필드·흐름이 없고, 48시간/30일 정책을 실행하는 scheduler도 없다.

`daese_prepare_cleanup`은 오래된 미완료 업로드 작업과 요청 제한 기록을 정리할 뿐이며 개인정보 보유기간 파기 기능이 아니다. 정책 이행이 검증되기 전 실제 접수를 열지 않는다.
...
구현 시 기준 시각·삭제 기한·처리 상태·시도 횟수·마지막 오류를 기록하고, DB 삭제와 사진 queue 처리, 주기 실행, 실패 재시도·경보, 경계 시간 테스트를 포함한다. 백업과 로그의 삭제·보유 방식도 별도로 확인한다.

...
사진 삭제까지 완료된 파기 이력은 완료 시각부터 90일 후 자동 삭제한다. 사진 삭제가 해결되지 않은 기록은 원인 확인과 재시도를 위해 성공할 때까지 유지하고, 성공 후 90일 기한을 적용한다.
...
Cron 실행 이력과 실패 queue는 운영 중 계속 점검한다. 실패 알림 수신 주소는 운영자가 지정했으며 실제 메일 발송 제공자와 서버 Secret 연결은 남은 작업이다. 백업과 로그의 삭제·보유 방식도 별도로 확인한다.
...
- [ ] Firebase 전화번호 인증을 도입하면 Google로 전화번호가 전송·저장되는 흐름, 동의·국외이전 고지와 SMS 비용을 반영. 실제 SMS는 Spark가 아닌 Blaze 요금제가 필요함.
